...
public class NioServer implements Runnable {
	...
	private static boolean atomic=false;
	...
	//CHECK THAT socket!=s
	public void broadcast(SocketChannel s, byte[] data) throws InterruptedException {
		...
		synchronized(this.clients){	
			for(SocketChannel socket: clients){
				if(socket!=s||Consensus.getAtomic().getAtomicNonBlocking()){
	...
	public void run() {
		while (true) {
			try {
				...
				//System.out.println("Services running: "+ clients.size()+" | Atomic mode: "+ atomic);
	...
	public static void main(String[] args) {
		try {
			...
			//InetAddress.getByName("152.23.155.186")
			new Thread(new NioServer(null, 9090, worker)).start();
			while(true){
		    	  Scanner s = new Scanner(System.in);
		    	  String command = s.next();
		    	  if(command.toLowerCase().equals("atomic")){
		    		  atomic=!atomic;

		    	  }else if(command.toLowerCase().equals("mode")){
		    		  System.out.println("Atomic mode: "+ atomic);
		    	  }
		      }
		} catch (IOException e) {
			e.printStackTrace();
